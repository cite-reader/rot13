...
fn main() {
    ...
    let rotated: Box<Iterator<Item=String>> =
        if matches.is_present("from-clipboard") {
...
fn rotate<I: IntoIterator<Item=io::Result<String>>>(input: I)
                                                -> Rotated<I::IntoIter> {
    Rotated {
...
impl<I: Iterator<Item=io::Result<String>>> Iterator for Rotated<I> {
    ...
    fn next(&mut self) -> Option<String> {
        self.inner.next().map(Result::unwrap).map(
            |s| s.chars().map(|c|
                              if c >= 'A' && c <= 'Z' {
